...
/**
 * Interacts with the main menu button event listeners and uses methods of
 * ClientController to produce appropriate output.
 * 
...
public class GUIController {
	...
	/**
	 * Contains all of the Action listeners for the buttons in the main menu.
	 * Produces appropriate output using ClientController methods.
	 */
		...
		myView.addCourse.addActionListener((ActionEvent e) -> {
			...
			if (exists == 1)
				...
				if (studentEnroll == 1)
					...
					client.socketOut
							.println("6 5 " + studentName + " " + courseName + " " + courseNumber + " " + section);
					int enroll = client.recieveInteger();

					if (enroll == 0) {
						client.socketOut
								.println("2 " + studentName + " " + courseName + " " + courseNumber + " " + section);
